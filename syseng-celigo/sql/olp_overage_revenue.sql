with olp_overage_revenue as (
  ...
  inner join `digital-arbor-400`.`pg_public`.`accounts` on revenue_records.billing_account_id = accounts.account_id
  where (
  ...
  left join `private-internal`.`netsuite2`.`customer` on olp_overage_revenue.billing_account_id = customer.custentity_ft_account_id_sf
    and not customer._fivetran_deleted
  left join `private-internal`.`netsuite2`.`partner` on customer.partner = partner.id
  ...
  left join `private-internal`.`netsuite2`.`item` on revenue_table_with_netsuite_id.account_tier = item.custitem_account_tier
