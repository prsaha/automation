...
--           Do NOT deploy this file to production — staging refs only.
...
with stripe_customers as (
  ...
  from `dulcet-yew-246109`.`stripe`.`customer` stripe_customer
  where not stripe_customer.is_deleted
),
-- Step 2: Active NetSuite sandbox customers, pre-filtered before joining.
...
--         Path B — new RunCache hub (staging project).
billing_ids_by_product_line as (
  ...
  from `dulcet-yew-246109`.`staging_billing_public`.`accounts` fivetran_account
  ...
  inner join `dulcet-yew-246109`.`staging_billing_public`.`run_cache_accounts` run_cache_account
...
where stripe_customers.stripe_netsuite_customer_id is null
  ...
  and netsuite_id_by_stripe.billing_account_id = 'onstage_cystic'
