{{
  config(
    ...
    alias        = 'syseng_run_cache_monthly_billing'
  )
}}
...
-- Step 2: Pre-filter active NetSuite customers.
--         isinactive = 'F': not deactivated, entitystatus = 13: customer status, isperson = 'F': company only.
active_netsuite_customers as (
  ...
  where not netsuite_customer._fivetran_deleted
    ...
    and netsuite_customer.isperson = 'F'
),
-- Step 3: Resolve the RunCache item from NetSuite.
netsuite_run_cache_item as (
  select
    netsuite_item.id
  from {{ source('netsuite2', 'item') }} netsuite_item
...
credits_by_billing_account_and_month as (
  ...
  from {{ source('staging', 'stg_pg_public_revenue_records') }} revenue_records
