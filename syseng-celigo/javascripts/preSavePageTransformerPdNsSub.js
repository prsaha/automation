...
// - RD-1010532: banding only for allowed SKUs, compute term_months (ceil), term_band,
//   map to NetSuite Item **internal id** via LOOKUP_NS_ITEMS into product_code_branding
//   (we DO NOT change product_code; product_code_branding holds the NS internal id when banding applies)
...
// - RD-927162: Suggest Billing Schedules per line based on Contract vs Service Start Dates and Billing Frequency
...
function preSavePage(options) {
    ...
    const CONFIG = {
        ...
        // RD-1010532: Lookup -> NetSuite Item INTERNAL IDs only
        LOOKUP_NS_ITEMS: {
            'ELA-On-Prem-Only': {
                '1 Year': 17957,
                '2 Year': 17958,
    ...
    'penance_traceable': 'Edgematics (Partner)',
    'lot_grist': 'Matrix IT Ltd (Partner)',
    'snow_mit': 'Devoteam EMEA (Partner)',
    'heavenly_helplessly': 'HITACHI SOLUTIONS EAST JAPAN, LTD. (Partner)',
    'acquirements_engraved': 'Bechtle Ltd (Partner)',
    'repugnance_lump': 'Intellection Corporation (Partner)',
    'estrangement_preliminaries': 'SCC FRANCE (Partner)',
    'disc_swim': 'BlueShift Brasil (Partner)',
    'notified_spent': 'CTCSP Corporation (Partner)',
    'printer_dazzled': 'Fujitsu Limited 富士通株式会社 (Partner)'
        },

        // ===== RD-927162: Billing Schedule suggestion config =====
        BILLING_SCHEDULES: {
            ANNUAL: {
                Y1: 'Annual - Yr 1',
                Y2: 'Annual - Yr 2',
    ...
    (options.data || []).forEach(function (record, index) {
        ...
        // ===== RD-1010532: banding only for allowed products =====
        (function applyElaHvrIfAllowed() {
            ...
            // Lookup internal id
            var entry = CONFIG.LOOKUP_NS_ITEMS[pc];
            if (!entry || !entry[band]) {
                newErrors.push({
                    recordIndex: index,
                    record: record,
                    message: "Lookup NS internal id not found for product_code='" + pc + "', term_band='" + band + "'.",
                    code: 'lookup_item_missing',
                    source: 'validation'
                });
                cleaned.alloc_strategy = 'band_missing';
                return;
            }
            // Success → set internal id only
            var internalId = entry[band];
            cleaned.product_code_branding = internalId; // NS Item internal id
        ...
        // ===== RD-927162: Auto-set Billing Schedules based on Contract vs Service Start Dates =====
            ...
            try {
                ...
                if (!cStart || !sStart) {
                    ...
                    if (diffMonths < 0) {
                        ...
                        var normalizedFreq = freq.replace(/_/g, ' ');

                         // Check more specific patterns first to avoid false matches
                            if (normalizedFreq.indexOf('SEMI') >= 0 || normalizedFreq.indexOf('HALF') >= 0) {
                                familyKey = 'SEMI_ANNUAL';
                            } else if (normalizedFreq.indexOf('QUARTER') >= 0) {
                                familyKey = 'QUARTERLY';
                            } else if (normalizedFreq.indexOf('ANNUAL') >= 0 || normalizedFreq.indexOf('UPFRONT') >= 0 || normalizedFreq.indexOf('UP FRONT') >= 0) {
                                familyKey = 'ANNUAL';
                            } else if (normalizedFreq.indexOf('MONTH') >= 0) {
                                familyKey = 'MONTHLY';
                            }
            ...
            removedOrders.map(function (o) {
                return {
                    message: "Didn't compute order " + o.order_number + " for " + o.billing_account_id +
                        " due to zero or negative total amount (" + o.total_amount + "). These subscriptions are rejected in Celigo.",
                    code: 'zero_or_negative_total',
                    source: 'BQ'
                };
            })
        );
    ...
    dbg('METRICS', metrics);
    console.log('[preSavePage:metrics RD-1010532 RD-899810 RD-1029971 RD-1060067 RD-927162]', JSON.stringify(metrics));
